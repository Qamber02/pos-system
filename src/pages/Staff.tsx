        ...
        <div className="min-h-screen bg-background flex flex-col">
            <header className="border-b bg-card shadow-sm sticky top-0 z-50">
                <div className="container mx-auto px-4 py-4 flex items-center gap-4">
                    <Navigation />
                    <h1 className="text-2xl font-bold">Staff & Access Control</h1>
                </div>
            </header>
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                ...
                                Store Staff & Operator Roles
                            </CardTitle>
                            <CardDescription>
                                Manage user permissions. Admins have complete control over settings and data; standard users operate POS, repairs, and inventory.
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                        ...
                                        <TableRow key={member.id}>
                                            <TableCell className="font-medium">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <div>
                                                        <div className="font-semibold">{member.full_name || "Staff Member"}</div>
                                                        <div className="text-xs text-muted-foreground">{member.role === 'admin' ? 'Administrator (Full Access)' : 'Cashier / Operator'}</div>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            <TableCell className="text-sm font-mono">{member.email}</TableCell>
                                            <TableCell>
                                                <Badge variant={member.role === 'admin' ? "default" : "secondary"} className="capitalize">
                                                    {member.role === 'admin' ? '👑 Admin' : '👤 Cashier / Staff'}
                                                </Badge>
                                            </TableCell>
